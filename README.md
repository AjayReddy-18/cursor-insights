...
- Multi-window coordination: only one Cursor window polls usage; alerts appear in the focused window
...
2. Download the latest `.vsix` asset (for example, `cursor-insights-1.3.0.vsix`).
...
The Conversation Insights section displays a doughnut chart for the selected metric and timeframe.
...
| Metric | Data source |
|--------|-------------|
| Work Type | Conversation segments — work type histogram |
| Intent Distribution | Conversation classification — intent distribution |
| Categories | Conversation segments — categories histogram |
| Task Complexity | Conversation classification — complexity distribution |
| Prompt Specificity | Conversation segments — guidance level distribution |

Percentages are calculated from the raw counts returned by the API.
...
When multiple Cursor windows are open, only one window polls usage. Alerts are delivered to the window that is focused at the moment a threshold crossing is detected.
